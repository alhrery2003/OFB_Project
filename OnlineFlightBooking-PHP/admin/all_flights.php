              ...
              <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                  ...
                  <tr class="text-center">                  
                    ...
                    <td>'.$row['Price'].'</td>
                    <td>
                    <form id="deleteForm-'.$unique_id.'" action="all_flights.php" method="post">
                      <input name="flight_id" type="hidden" value='.$row['flight_id'].'>
                      <button type="button" class="btn" type="submit" name="del_flight"
                      onclick="
                            var userConfirmed = confirm(\'Are you sure you want to Delete This Flight ?                                **NOTE THAT THIS WILL ALSO DELETE EVERYTHING ELSE RELATED TO THIS FLIGHT INCLUDING ROUND FLIGHT AND PASSENGERS**\');
                            if (userConfirmed) {
                              var form = document.getElementById(\'deleteForm-' . $unique_id . '\');
                              var input = document.createElement(\'input\');
                              input.type = \'hidden\';
                              input.name = \'del_flight\';
